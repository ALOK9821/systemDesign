interface Location {
  ...
  class RideSharingService {
    ...
    requestRide(riderId: string, endLocation: Location): Ride | null {
      ...
      const nearestDriver = findNearestDriver(rider, Array.from(this.drivers.values()));
      if (!nearestDriver) {
  ...
  const driver1 = new Driver('d1', 'Driver 1', { latitude: 40.7128, longitude: -74.0060 }, true);
  const driver2 = new Driver('d2', 'Driver 2', { latitude: 40.7127, longitude: -74.0059 }, true);
  const rider = new Rider('r1', 'Rider 1', { latitude: 40.7126, longitude: -74.0061 });
  
  service.addDriver(driver1);
  service.addDriver(driver2);
  service.addRider(rider);
  
  const ride = service.requestRide('r1', { latitude: 40.7125, longitude: -74.0062 });
  if (ride) {
